...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("slash commands", () => {
		...
		it("does not surface skills for a leading prefix unrelated to the namespace", async () => {
			...
			// Only bare-name (or hyphen-segment) prefixes break a skill out of the
			// collapsed namespace at prompt start: `hmz` merely fuzzy-matches
			// `humanizer`, and description hits never count, so the popup stays shut.
			for (const input of ["/hmz", "/writing"] as const) {
				const result = await provider.getSuggestions([input], 0, input.length);
				expect(result).toBeNull();
			}
		});
