...
// Every signal `classifyTerminalMultiplexer` accepts: the Herdr identity vars
// (which survive env-sanitizing launchers that drop HERDR_ENV) and the
// CMUX/WMUX markers included, or a run inside one of those panes classifies as
// a multiplexer and keeps the DSR probe this suite asserts is suppressed.
const TERMINAL_ENV = [
	"TERM",
	"TERM_PROGRAM",
