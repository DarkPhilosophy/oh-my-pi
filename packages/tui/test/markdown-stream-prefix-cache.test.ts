...
describe("Markdown streaming prefix render cache", () => {
	...
	it("matches a cold render when a live table grows after a cached table", () => {
...
| tail-column-is-even-longer-than-before | R001 |`;
		...
		expect(expandedLines).toEqual(renderCold(expandedText, defaultMarkdownTheme));
		expect(expandedTail.startRow).toBe(tailTable.startRow);
