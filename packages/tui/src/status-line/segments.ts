	...
	render(ctx) {
		...
		if (stripPrefix && ctx.worktree) {
			...
			const text = ctx.startupPlaceholder
				? STARTUP_PLACEHOLDER
				: fileHyperlink(ctx.projectDir, clampPathLength(label, opts.maxLength ?? 40));
			const content = withIcon(theme.icon.worktree, text);
			return { content: theme.fg("statusLinePath", content), visible: true };
		}

		// ctx.projectDir is the session-owned cwd; the process-global
		// getProjectDir() belongs to whatever scope the render fired from
		// (daemon socket callback, TUI timer) and makes this segment flicker.
