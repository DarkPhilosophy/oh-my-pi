...
import { type DaemonConnectionSnapshot, formatDaemonWelcomeStatus } from "../chrome/daemon-status";
import {
...
interface CachedStatusLine {
	...
	/** Absent only for the sessionless empty bar, which is never cached. */
	externalInputs: StatusLineExternalInputs | undefined;
}
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	#getStatus(activeRepoCache: ActiveRepoCache = this.#resolveActiveRepoCache()): {
		...
		// Idle sessions reuse the cached summary; the one exception is the first
		// idle render after a turn, so edits from its final seconds still show.
		const streaming = this.session.isStreaming === true;
		const turnJustEnded = this.#gitStatusSawStreaming && !streaming;
		if (streaming) this.#gitStatusSawStreaming = true;
		if (
			this.#cachedGitStatusCwd === gitCwd &&
			!turnJustEnded &&
			(!streaming || Date.now() - this.#gitStatusLastFetch < GIT_STATUS_REFRESH_TTL_MS)
		) {
			return this.#cachedGitStatus;
		}
		this.#gitStatusSawStreaming = streaming;
	...
	#readStatusLineExternalInputs(): StatusLineExternalInputs {
		...
		return {
			...
			// Session-owned cwd, matching what the path/git segments render.
	...
	#sameStatusLineExternalInputs(left: StatusLineExternalInputs | undefined, right: StatusLineExternalInputs): boolean {
	...
	#buildStatusLine(
		...
		// A remote/daemon projection can exist before its session is attached.
		// Every input read below dereferences the session, so short-circuit to an
		// empty bar (and skip the render cache, which is keyed on those inputs).
		if (!this.session) {
			return {
				content: "",
				dimmedContent: "",
