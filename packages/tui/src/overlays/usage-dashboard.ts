...
export interface ProviderCard {
	...
	idle: boolean;
	/** Banked reset credits across this card's accounts, when any remain. */
	resetCredits?: {
		bankedCount: number;
...
export function buildProviderCards(
	...
	for (const { provider, account, reports: providerReports } of grouped.values()) {
		...
		// A spent grant keeps its expiry in the snapshot; the card must show the
		// soonest expiry that is still ahead, falling back to the latest known one.
		...
		const resetCredits =
			bankedCount > 0
				...
				: undefined;
		// Same identity fallback and privacy mask as per-account card headers.
...
export class UsageDashboardComponent implements Component {
	...
	#renderCardLines(card: ProviderCard, width: number): string[] {
		...
		for (const account of card.daybreakAccounts ?? []) {
			const label = this.#styleMask(sanitizeText(account.replace(/[\r\n\t]+/g, " ")));
