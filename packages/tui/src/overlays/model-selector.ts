...
export function supportsUpstreamRouting(model: { provider: string; baseUrl: string }): boolean {
	return modelMatchesHost(model, "openrouter") || modelMatchesHost(model, "vercelAIGateway");
}

/**
 * Parse a registry-backed exact selector without falling back to fuzzy model
 * matches. A registered literal id wins before `@` is read as an upstream
 * route and before a trailing `:effort` is split (`google-vertex/claude-opus-4-8@default`
 * and `test/nanogpt/coding-router:high` are real models, not routes/efforts);
 * `@upstream` is only honored for hosts that support upstream routing, so
 * `anthropic/claude-sonnet-4-5@cerebras` resolves to nothing.
 */
export function parseExactModelSelectorWithRouting<M extends { provider: string; id: string; baseUrl: string }>(
	pattern: string,
	findModel: (provider: string, id: string) => M | undefined,
