...
import { parseModelString, parseExactModelSelectorWithRouting, formatModelSelectorValue } from "./model-selector";
import {
	buildBrowserItems,
	ModelBrowser,
...
export class ModelHubComponent implements Component {
	...
	/** Split a fallback-chain entry into its model base, explicit effort, and `@upstream` routing. */
	...
	/**
	 * Canonical spelling of a fallback-chain entry for duplicate/primary
	 * comparison: trimmed when it parses as a model selector, verbatim
	 * otherwise (wildcards and role keys compare as written).
	 */
	#normalizeChainSelector(selector: string): string {
		...
		return parsed ? trimmed : selector;
	}
	...
	#setFallbackThinking(role: string, index: number, level: ConfiguredThinkingLevel): void {
		...
		const normalizedNext = this.#normalizeChainSelector(next);
		const primaryRaw = role.includes("/") ? role : this.#settings.getModelRole(role);
		const normalizedPrimary = primaryRaw ? this.#normalizeChainSelector(primaryRaw) : undefined;
		if (role !== "default" && normalizedPrimary === normalizedNext) {
			chain.splice(index, 1);
		} else {
			chain[index] = next;
			for (let i = chain.length - 1; i >= 0; i--) {
				if (i !== index && this.#normalizeChainSelector(chain[i] ?? "") === normalizedNext) chain.splice(i, 1);
			}
		}
