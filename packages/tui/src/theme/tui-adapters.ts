...
import { clearRenderCache, type MarkdownTheme } from "../components/markdown";
...
/** Install the host's copy-target builder (see `utils/copy-store`). */
export function setCopyUrlTargetProvider(
	provider: ((code: string, handlerReady: boolean) => string | undefined) | undefined,
): void {
	copyUrlTarget = provider;
	clearRenderCache();
}
