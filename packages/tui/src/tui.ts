...
/** Extra zero-width rows an OSC 66 scaled heading reserves below itself. */
function getOsc66ReservedRows(line: string): number {
	...
	for (let m = OSC66_SPAN_REGEX.exec(line); m !== null; m = OSC66_SPAN_REGEX.exec(line)) {
		...
		for (const part of m[1]!.split(":")) {
			...
			if (part[0] === "s" && value >= 1 && value <= 7) scale = value;
		}
...
export interface TerminalFramePlan {
	 ...
	 * Consumed only by right-panel placement targeting; omitting it makes every
...
export class TUI extends Container {
	...
	#cursorOverlayBacking:
		...
		| undefined;
	/** Uncovered physical screen used to restore graphics and remap resize backing. */
	...
	#resizeProbe:
		...
		| undefined;
	// Right-side info panel state. The provider is consulted per frame and its
	// blocks are composited into the visible viewport's trailing whitespace.
	...
	// Row ownership published by the last provider plan; drives target/exclusion
	// placement for the right panel only.
	#planSegments: readonly TerminalFrameSegment[] = [];
	// Rows composed directly above the viewport this frame (children path only).
	// Lets the right-panel compositor see a scaled OSC 66 heading whose
	// reservation rows reach into the window's first rows.
	#rightPanelRowsAbove: readonly string[] = [];
	// True once start() has run. A provider registered during setup must not
	// paint before the terminal is started, or it commits a frame into raw
	// scrollback before the screen is cleared.
	#hasStarted = false;
	...
	#trackResizeBurst(): void {
		...
		if (
			backing &&
			!isInsideTerminalMultiplexer() &&
			this.terminal.rows < burstLastHeight &&
			backing.top < this.#providerViewportTop + this.#providerWindow.length - this.terminal.rows
		) {
			// Capture damage before resize paints replace the old viewport geometry.
	...
	#remapCursorOverlayBacking(width: number, height: number, viewportTop: number, anchorKnown = true): void {
		...
		this.#providerVisibleHistory = screen.slice(0, viewportTop);
	...
	#flushHistoryBeforeStop(): void {
		...
		if (!provider.beginHistoryFlush) this.#cursorOverlayRender = undefined;
		if (this.#cursorOverlayResizePending || this.#cursorOverlayHistoryDamaged) {
			this.#prepareResizeReplay(width, height);
		}
		while (
			this.#renderProviderFrame(
				width,
				height,
				provider.beginHistoryFlush !== undefined || this.#clearScrollbackWaitsForReplay,
			)
		) {
			// Drain queued append batches until the complete recovery replay arrives.
			if (!provider.beginHistoryFlush && !this.#clearScrollbackOnNextRender) break;
		}
	}
	...
	requestComponentRender(_component: Component): void {
		if (this.#stopped) return;
		// A self-contained component change (streaming row, spinner) is live
		// output: keep it on the larger backlog budget instead of the ordinary
		// gate, which would stall a fast reply mid-stream.
	 ...
	 * viewport. `targets` opt rows in; `exclusions` opt rows out and always win,
	...
	setRightPanel(
		...
		// during setup would otherwise commit a frame into raw scrollback before
		// the screen is cleared. start() does the initial paint.
		if (this.#hasStarted) this.requestRender();
	}

	/**
	 * Composite the registered right panel into the mutable viewport. These rows
	...
	#compositeRightPanelIntoViewport(viewport: string[], width: number, overlayVisible: boolean): string[] {
		...
		// A modal overlay owns the screen; the panel contract is all-or-hidden.
		if (overlayVisible) {
			hideAll();
			return viewport;
		}
		let lo = 0;
		let hi = viewport.length;
		...
		if (targets !== null) {
			...
			for (const segment of this.#planSegments) {
				...
				for (let row = Math.max(0, segment.start); row < end; row++) eligibleRows[row] = true;
			}
			...
			if (exclusions !== null) {
				for (const segment of this.#planSegments) {
					...
					const end = Math.min(viewport.length, segment.start + segment.rowCount);
					for (let row = Math.max(0, segment.start); row < end; row++) eligibleRows[row] = false;
				}
			}
			let firstEligible = -1;
			let lastEligible = -1;
		...
		// Kitty OSC 66 headings reserve zero-width rows immediately below them;
		// painting there erases the scaled glyph (#8318).
		for (let row = 0; row < viewport.length; row++) {
			...
			for (let offset = 1; offset <= reservedRows; offset++) {
				if (row + offset >= viewport.length) break;
				if (visibleWidth(viewport[row + offset] ?? "") !== 0) break;
				occupied[row + offset] = true;
			}
		}
		// A scaled heading can sit above the viewport with its reservation rows
		// reaching into it. Walk back through the rows scrolled off this frame
		// (children path) so those rows stay unpainted too (#8318).
		const above = this.#rightPanelRowsAbove;
		for (let gap = 1; gap <= above.length; gap++) {
			const line = above[above.length - gap] ?? "";
			if (isOsc66Line(line)) {
				const spill = getOsc66ReservedRows(line) - (gap - 1);
				for (let row = 0; row < spill && row < viewport.length; row++) {
					if (visibleWidth(viewport[row] ?? "") !== 0) break;
					occupied[row] = true;
				}
				break;
			}
			// Only a run of zero-width reservation rows can separate the heading
			// from the window; real content ends the lookback.
		...
		const composited = compositeRightPanelsInRange(
			...
			lo,
			hi,
			(line, index): boolean => occupied[index] === true || TERMINAL.isImageLine(line),
			line => TERMINAL.isImageEscapeLine(line),
			this.#rightPanelLayoutCallback ?? undefined,
		);
		return composited === viewport ? viewport : composited;
	}
	...
	#renderProviderFrame(width: number, height: number, flushing = false): boolean {
		...
		if (this.#clearScrollbackOnNextRender && !this.#clearScrollbackWaitsForReplay) {
			this.#providerLogicalCommitted = 0;
			this.#providerExpansionBorrowed = false;
			this.#providerHasTransientHistory = false;
			this.#providerTransientRows = [];
	...
	#emitPlanFrame(
		...
		if (history?.kind === "replay" && retainedHistory === undefined) {
			...
			if (viewport.length < height) {
				viewport = [...Array.from({ length: height - viewport.length }, () => ""), ...viewport];
			}
		...
		const prefixRows = viewport.length - viewportRows.length;
		if (prefixRows > 0) {
			this.#planSegments = this.#planSegments.map(segment => ({ ...segment, start: segment.start + prefixRows }));
			this.#rightPanelRowsAbove = [];
		}
		const overlayVisible = !flushing && this.#getTopmostVisibleOverlay() !== undefined;
		if (overlayVisible) {
			while (viewport.length < height) viewport.push("");
	...
	#renderChildrenFrame(width: number, height: number): void {
		...
		// publishes the same row-ownership map the provider path supplies.
		const segments: TerminalFrameSegment[] = [];
		if (this.#rightPanelProvider !== null) {
			let start = 0;
			for (const child of this.children) {
				const rowCount = child.render(width).length;
				segments.push({ component: child, start, rowCount });
				start += rowCount;
			}
		}
		this.#debugNextWindowTop = Math.max(0, composed.length - height);
		...
		this.#planSegments = segments.map(segment => ({
			component: segment.component,
			start: segment.start - scrolledOff,
			rowCount: segment.rowCount,
		}));
