	...
	const mode =
		...
		(/^\*\*\* (?:Add|Update|Delete) File:/m.test(input)
			...
			: /^\s*<SM:/im.test(input)
				...
				: undefined);
	try {
		// No render args here (path-only query): use a throwaway carrier for the memo slot.
		const entries =
			mode && mode !== "hashline"
				? inspectInputEntries({} as EditRenderArgs, mode, input)
				: getHashlineInputSections(input);
		const paths: string[] = [];
