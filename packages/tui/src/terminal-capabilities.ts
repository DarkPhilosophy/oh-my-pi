...
export class TerminalInfo {
	...
	isImageEscapeLine(line: string): boolean {
		...
		if (protocol === ImageProtocol.Sixel) {
			const limit = Math.min(line.length, SIXEL_MARKER_SCAN_LIMIT);
			for (let i = 0; i < limit; i++) {
				if (line.charCodeAt(i) === 0x1b && this.hasImageMarkerAt(line, i)) return true;
			}
			return false;
		}
		// Placeholder rows are the visible grid; only the data escape reserves cells.
		const index = line.indexOf(protocol);
		return index !== -1 && index + protocol.length <= 64;
	}
