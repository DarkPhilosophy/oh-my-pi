	...
	it("keeps the eval worker graph importable after computer renderer registration", async () => {
		const processHandle = Bun.spawn(
			[
				process.execPath,
				"-e",
				'await import("./src/eval/js/context-manager.ts"); const { toolRenderers } = await import("../tui/src/tools/index.ts"); if (typeof toolRenderers.hub.renderCall !== "function") process.exit(2)',
			],
		...
		if (exitCode !== 0) throw new Error(`computer worker module graph smoke test failed: ${stderr}`);
