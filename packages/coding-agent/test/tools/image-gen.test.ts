...
function createAntigravityXAIContext(model: Model | undefined, fetchMock: typeof fetch): CustomToolContext {
	...
	return {
		...
		modelRegistry: {
			...
			getAll: () => [],
			authStorage: {
				keys: { source: (provider: string) => (provider === "xai-oauth" ? { concrete: true } : undefined) },
...
describe("imageGenTool", () => {
	...
	it("skips DeepInfra for edit requests so an edit-capable provider can serve them", async () => {
		...
		// Gemini resolves GOOGLE_API_KEY straight from the environment, so an
		// ambient developer key would otherwise serve the edit and hide the
		// DeepInfra skip this test pins.
