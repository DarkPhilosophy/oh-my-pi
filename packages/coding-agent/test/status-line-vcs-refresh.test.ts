...
describe("StatusLineComponent repaints when an async VCS fetch resolves", () => {
	...
	it("refreshes git status once when a turn ends, then stays idle", async () => {
		let now = 1_000_000;
		vi.spyOn(Date, "now").mockImplementation(() => now);
		...
		// The turn ends inside the active-session TTL: edits from its last
		// seconds must still reach the idle status line.
		now += 1_000;
