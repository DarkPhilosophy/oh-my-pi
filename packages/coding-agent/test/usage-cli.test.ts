...
describe("computeProviderWindowStats", () => {
	...
	it("does not zero out an account whose model-scoped window is pinned while shared quota has headroom", () => {
		...
		// 7d window at 17% must leave 0.17 on the shared window, with Spark as its own meter.
		const reports = [
		...
		const stats = computeProviderWindowStats(reports);
		const shared = stats.find(stat => stat.meter === undefined || stat.meter === "chat");
		const spark = stats.find(stat => stat.meter === "spark");
		expect(shared?.usedAccounts).toBeCloseTo(0.17);
		expect(shared?.remainingAccounts).toBeCloseTo(0.83);
		expect(spark?.usedAccounts).toBeCloseTo(1);
	});
