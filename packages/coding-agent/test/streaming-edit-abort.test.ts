...
describe("streaming edit abort", () => {
	...
	test("aborts on an error from the native final patch preview", () => {
		const { guard, aborts } = createGuard(
			true,
			process.cwd(),
			Settings.isolated({ "edit.mode": "patch", "edit.streamingAbort": true }),
		);
		guard.maybeAbort(
			previewEvent(false, [
				{ path: "src/ok.ts" },
	...
	test("hashline final-preview errors stay tool-local even when streaming abort is enabled", () => {
		const settings = Settings.isolated({ "edit.mode": "hashline", "edit.streamingAbort": true });
		const { guard, aborts } = createGuard(true, process.cwd(), settings);
		guard.maybeAbort(previewEvent(false, [{ path: "a.ts", error: "invalid hashline range" }]));
		expect(aborts.count).toBe(0);
		expect(guard.abortTriggered).toBe(false);
	});

	test("uses the active edit mode rather than the persisted settings mode", () => {
		const settings = Settings.isolated({ "edit.mode": "patch", "edit.streamingAbort": true });
		const aborts = { count: 0 };
		const activeModeHost = {
			agent: {
				abort() {
					aborts.count++;
				},
			...
			settings,
			sessionManager: { getCwd: () => process.cwd() } as SessionManager,
			obfuscator: undefined,
			model: () => undefined,
			resolveActiveEditMode: () => "hashline" as const,
			isDisposed: () => false,
			promptGeneration: () => 0,
			localProtocolOptions: () => ({}),
			emitNotice() {},
		...
		expect(aborts.count).toBe(0);
		expect(activeHashlineGuard.abortTriggered).toBe(false);
	});

	test("reset permits a later final patch preview to abort", () => {
		const { guard, aborts } = createGuard(
			true,
			process.cwd(),
			Settings.isolated({ "edit.mode": "patch", "edit.streamingAbort": true }),
