...
import { InternalUrlFilesystem, isUrlPath, UrlFsError } from "../internal-urls/url-filesystem";
...
import {
	...
	cfgBashInterceptorForwardSimpleCommands,
...
export class BashTool implements AgentTool<BashToolSchema, BashToolDetails> {
	...
	async execute(
		...
		if (cfgBashInterceptorEnabled.get(this.session.settings)) {
			...
			const forwardSimpleCommands = cfgBashInterceptorForwardSimpleCommands.get(this.session.settings);
			...
			for (const commandToCheck of commandsToCheck) {
				...
				const hasForwardUnsupportedOptions =
					...
					pty ||
					rawCommand.includes("://") ||
					(rawTimeout !== undefined && rawTimeout !== 300);
		...
		// Command text and URL cwds resolve through the embedded shell's URL
		// filesystem; env values are plain strings handed to the process, so an
		// env value naming an internal URL is located to its host path here.
		const resolvedEnv = env
			? Object.fromEntries(
					await Promise.all(
						Object.entries(env).map(async ([key, value]) => {
							if (!isUrlPath(value)) return [key, value];
							const located = await InternalUrlRouter.instance().locate(
								value,
								sessionResolveContext(this.session, { signal }),
								{ create: true },
							);
							return [key, located ?? value];
						}),
					),
				)
			: undefined;
