...
let firefoxSharedTab: WorkerTabSession | undefined;
...
async function acquireTabImpl(
	...
	if (browser.kind.kind === "firefox-relay" && firefoxSharedTab?.state === "alive") {
	...
	tabs.set(name, tab);
	if (tab.kindTag === "firefox-relay") firefoxSharedTab = tab;
	return { tab, created: true };
}
...
async function forceKillTab(name: string, reason: string): Promise<void> {
	...
	if (firefoxSharedTab?.worker === tab.worker) firefoxSharedTab = undefined;
	await tab.worker.terminate().catch(() => undefined);
