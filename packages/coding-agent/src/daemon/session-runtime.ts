...
export type DaemonSession = {
	...
	readonly agent?: {
		state?: { messages?: readonly unknown[]; systemPrompt?: string[]; tools?: readonly unknown[] };
	...
	readonly setSteeringMode?: (mode: "all" | "one-at-a-time") => void;
	readonly setFollowUpMode?: (mode: "all" | "one-at-a-time") => void;
...
function isExtensionUIResponse(
	value: unknown,
): value is Record<string, unknown> & { type: "extension_ui_response"; id: string } {
...
async function prepareCliLaunch(
	...
	const activeSettings = await Settings.init({ cwd, configFiles: parsed.config });
	...
	if (parsed.smol || parsed.slow || parsed.plan) {
		activeSettings.overrideModelRoles({ smol: parsed.smol, slow: parsed.slow, plan: parsed.plan });
	}
...
export async function createAgentSessionRuntime(
	...
	const emitAvailableCommands = async (): Promise<void> => {
		emitBridgeEvent({ type: "available_commands_update", commands: await getAvailableCommands() });
	};
	...
	const extensionUiContext = {
		...
		select: async (title: string, options: Array<{ label: string } | string>) => {
			...
			const response = await extensionDialog({ method: "select", title, options: labels });
		...
		confirm: async (title: string, message: string) => {
			const response = await extensionDialog({ method: "confirm", title, message });
			return response.cancelled !== true && response.confirmed === true;
		},
		input: async (title: string, placeholder?: string) => {
			const response = await extensionDialog({ method: "input", title, placeholder });
			return response.cancelled === true
				? undefined
				: typeof response.value === "string"
					? response.value
		...
		setTitle: (title: string) =>
			emitBridgeEvent({ type: "extension_ui_request", id: crypto.randomUUID(), method: "setTitle", title }),
		custom: async () => undefined,
		setEditorText: (text: string) =>
			emitBridgeEvent({ type: "extension_ui_request", id: crypto.randomUUID(), method: "set_editor_text", text }),
		...
		setTheme: async () => ({ success: false, error: "Theme changes are client-owned" }),
		...
		mode = new InteractiveMode(
			...
			result.eventBus,
			{
				...
				onDetach: (reason, error) => {
					...
					emitBridgeEvent({ type: "terminal_closed", reason, ...(error === undefined ? {} : { error }) });
				},
			},
		);
			...
			try {
				if (!startupInputsSubmitted) {
					...
					if (initialMessage !== undefined) {
						await result.session.prompt(initialMessage, { images: initialImages });
					}
			...
			switch (command.type) {
				...
				case "execute_slash_command": {
					...
					const slashResult = await executeAcpBuiltinSlashCommand(command.text, {
						...
						output: text => emitBridgeEvent({ type: "notice", level: "info", message: text }),
				...
				case "new_session":
					return { cancelled: !(await result.session.newSession({ parentSession: command.parentSession })) };
				case "switch_session":
					return { cancelled: !(await result.session.switchSession(command.sessionPath)) };
				case "branch": {
