...
export class AdvisorConfigOverlayComponent implements Component {
 ...
 #showToolsEditor(scope: AdvisorConfigScope, index: number, selected: Set<string>, cursor: number): void {
  ...
  const advisor = this.#scopes[scope].doc.advisors[index];
  const apply = (): void => {
   const currentIndex = advisor ? this.#scopes[scope].doc.advisors.indexOf(advisor) : -1;
   if (currentIndex < 0) {
    this.#showFields();
    return;
   }
   this.#scopes[scope].doc.advisors[currentIndex].tools = commitTools(selected, all);
   this.#markDirty(scope);
   this.#showFields();
  };
  list.onSelectionChange = item => {
  ...
  list.onSelect = item => {
   ...
   this.#showToolsEditor(scope, this.#scopes[scope].doc.advisors.indexOf(advisor), selected, cursorIndex);
  };
  list.onCancel = apply;
