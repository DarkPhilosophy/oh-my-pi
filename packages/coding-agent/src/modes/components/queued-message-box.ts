 ...
 *
 * The streaming steer box omits its own top rule (`showTopBorder: false`):
 * the animated {@link SteeringIndicator} supplies `╭╌[   Steering   ]╌╮` and this
 * box stacks beneath it, body + bottom only, column-for-column aligned.
 */
const DASH = "╌";
const VDASH = "╎";
...
export interface QueuedMessageBoxOptions {
	...
	expanded: boolean;
	/** When false, omit the top rule — the caller (SteeringIndicator) supplies it. */
...
export class QueuedMessageBox implements Component {
	...
	#topBorder(width: number): string {
		...
		if (!this.#title) return paint(box.topLeft + DASH.repeat(inner) + box.topRight);
		const shown = truncateToWidth(` ${this.#title} `, Math.max(0, inner - 2));
		const fill = Math.max(0, inner - 1 - visibleWidth(shown));
		return (
			paint(box.topLeft + DASH) + theme.bold(theme.fg("accent", shown)) + paint(DASH.repeat(fill) + box.topRight)
		);
	}
	...
	#bottomBorder(width: number, hiddenRows: number, hiddenChars: number, footerText: string | undefined): string {
		...
		if (!seg) return paint(box.bottomLeft + DASH.repeat(inner) + box.bottomRight);
		...
		return (
			paint(box.bottomLeft + DASH.repeat(left)) +
			theme.fg("muted", segShown) +
			paint(DASH.repeat(right) + box.bottomRight)
		);
	}
}
