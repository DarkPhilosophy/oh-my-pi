...
import { formatDuration, logger, popLoopPhase, prompt, pushLoopPhase, sanitizeText } from "@oh-my-pi/pi-utils";
...
export class EventController {
	...
	constructor(private ctx: InteractiveModeContext) {
		...
		// tiny/smol role with the primary session's registry and credentials; the
		// vocalizer falls back to mechanical cleanup when unset.
	...
	async #handleMessageUpdate(event: Extract<AgentSessionEvent, { type: "message_update" }>): Promise<void> {
		...
		if (event.message.role === "assistant") {
			...
			if (!this.ctx.streamingComponent) {
				...
				this.ctx.streamingMessage = event.message;
				this.ctx.chatContainer.addChild(this.ctx.streamingComponent);
				const startTimeline = splitAssistantMessageToolTimeline(event.message);
				this.#streamingReveal.begin(
					this.ctx.streamingComponent,
					startTimeline.beforeTools,
					startTimeline.hasToolCalls,
				);
			}
			this.ctx.streamingMessage = event.message;
			const timeline = splitAssistantMessageToolTimeline(this.ctx.streamingMessage);
