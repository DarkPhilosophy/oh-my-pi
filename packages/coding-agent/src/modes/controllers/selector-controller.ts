...
export class SelectorController {
	...
	#showModelPicker(): void {
		...
		const inline =
			cfgDisplayInlineModelPicker.get(this.ctx.settings) && this.ctx.editorContainer.children.includes(editor);
	...
	async #redeemReset(account: ResetUsageAccount): Promise<void> {
		this.ctx.showStatus(`Spending 1 saved reset for ${sanitizeText(account.label.replace(/[\r\n\t]+/g, " "))}…`, {
			dim: true,
		});
