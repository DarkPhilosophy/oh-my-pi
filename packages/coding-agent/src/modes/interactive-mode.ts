...
export class InteractiveMode implements InteractiveModeContext {
	...
	#syncTodoHudState(owner: AgentSession): void {
		...
		const persistAndHide = async (): Promise<void> => {
			...
			this.#renderTodoList();
			// Hiding the HUD only shrinks the live frame. Clearing native scrollback
			// here replayed the whole session history (thousands of rows) every time
			// the list auto-dismissed — a flicker plus a jump to the bottom.
			this.ui.requestRender(true);
		};
