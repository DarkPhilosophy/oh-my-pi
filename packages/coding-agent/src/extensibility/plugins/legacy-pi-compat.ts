...
/**
 * Set while `Bun.resolveSync` below is running. The shim's own `onResolve`
 * filter matches canonical `@oh-my-pi/pi-*` specifiers, so the nested
 * resolution re-enters `resolveLegacyPiSpecifier`; Bun answers that recursion
 * by re-prefixing the pending specifier (`file:file:file:…/index.ts`) until the
 * name blows past PATH_MAX. Host-internal lazy `require("@oh-my-pi/pi-ai/…")`
 * boundaries hit exactly that path once any extension/plugin loader installed
 * the shim.
 */
...
/**
 * Directory holding the host's own sibling packages: `<repo>/packages` in dev
 * mode, `<…>/node_modules/@oh-my-pi` for an installed tree, `/$bunfs/root…`
 * inside a compiled binary. Host source files live under it; extension code
 * never does.
 */
const HOST_PACKAGE_SIBLING_ROOT = path.dirname(path.resolve(import.meta.dir, "..", "..", ".."));

/**
 * True for importers that are host source files. The shim exists to point
 * *extension* code at the in-process host copy; a host module resolving a
 * canonical `@oh-my-pi/pi-*` specifier already lands there through ordinary
 * workspace resolution, and routing it through the plugin makes Bun hand the
 * resolved path back as a `file:`-prefixed name it then fails to read.
 */
function isHostInternalImporter(importer: string): boolean {
	return importer.startsWith(`${HOST_PACKAGE_SIBLING_ROOT}${path.sep}`);
}
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): LegacyPiResolveResult | undefined {
	// Nested resolution from `getResolvedSpecifier`, or a host module resolving
	// its own sibling package: let Bun answer natively. Re-entering here makes
	// Bun hand the pending specifier back `file:`-prefixed instead of resolving
	// it, which is what broke host-internal lazy `require("@oh-my-pi/pi-*")`
	// boundaries once any extension loader installed the shim. A nested call
	// from our own `Bun.resolveSync` also declines, so Bun resolves natively
	// from the directory that call chose (the host location first, so canonical
	// imports keep landing on the host copy, not a plugin-local one).
	if (isResolvingLegacyPiSpecifier || isResolvingCanonicalPiSpecifier || isHostInternalImporter(args.importer)) {
		return undefined;
	}
