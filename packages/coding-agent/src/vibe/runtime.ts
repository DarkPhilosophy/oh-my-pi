...
/**
 * Registry-scoped collection of vibe worker sessions, scoped by both owner agent
 * id (same convention as AsyncJobManager owner filters) and stable parent session
 * id. The interactive mode kills an owner's sessions on vibe-mode exit via
 * {@link killAll}. Persisted lifecycle events rebuild idle records after a
 * process restart; live turn jobs remain process-local.
 */
