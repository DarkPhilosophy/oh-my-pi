...
export class Settings {
	...
	#migrateRawSettings(raw: RawSettings, captureLegacyChangelogVersion = true): RawSettings {
		...
		if (legacyUnexpectedStop !== undefined) {
			...
			if (!currentIsMode) target.unexpectedStopDetection = legacyUnexpectedStop ? "smart" : "none";
		...
		function migrateKindRoleSettings(): void {
			...
			// Local model the fork consulted only after the online classifier failed.
			const legacyUnexpectedStopFallbackModel = legacy(
				providerSettings,
				"unexpectedStopFallbackModel",
			...
			if (nonDefaultJudge) {
				...
				if (hasJudgeFallback) judgeCandidates.push(`local/${legacyUnexpectedStopFallbackModel}`);
