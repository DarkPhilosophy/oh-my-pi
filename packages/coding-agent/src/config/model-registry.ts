...
import type { ApiKeyResolver, FetchImpl, OAuthAccess, ResolvedApiKey, UsageProvider } from "@oh-my-pi/pi-ai";
...
export class ModelRegistry {
	...
	#loadBuiltInModels(overrides: Map<string, ProviderOverride>, providerFilter?: ReadonlySet<string>): Model<Api>[] {
		return getBundledProviders().flatMap(provider => {
			...
			return models.map(m => {
				const baseModel = buildModel(toModelSpec(m) as ModelSpec<Api>);
				if (!providerOverride) return baseModel;
				const withTransportOverride = this.#applyProviderTransportOverride(
					toModelSpec(baseModel),
					providerOverride,
				);
				return buildModel({
					...withTransportOverride,
					compat: mergeCompat(baseModel.compatConfig, providerOverride.compat),
				} as ModelSpec<Api>);
			});
		});
	}
	...
	async #refreshRuntimeDiscoveries(
		...
		for (let index = 0; index < discoveredCount; index++) {
			...
			if (strategy === "offline") {
				...
				try {
					available = (await this.authStorage.oauth.reserveCredential(model.provider, route)) !== undefined;
				} catch (error) {
	...
	#formatBuiltInOAuthDiscoveryApiKey(providerId: string, access: OAuthAccess | undefined): string | undefined {
	...
	async #resolveBuiltInDiscoveryAccess(providerId: string): Promise<OAuthAccess | undefined> {
		if (providerId === "github-copilot" || providerId === "gitlab-duo-agent") {
			const first = this.authStorage.oauth.accounts(providerId)[0];
			if (!first) return undefined;
			const resolution = await this.authStorage.oauth.accessById(providerId, first.credentialId);
			return resolution?.ok ? resolution : undefined;
		}
		return this.authStorage.oauth.access(providerId);
	}

	async #resolveBuiltInDiscoveryOAuthAccess(providerId: string): Promise<OAuthAccess | undefined> {
		try {
	...
	#resolveBuiltInDiscoveryCacheIdentity(providerId: string): string | undefined {
		...
		// oauth.accessById(providerId, <first row id>); both index the same stable storage
		// order, so a multi-account provider cannot cache one account's catalog
		// under another account's key.
		return formatOAuthCacheIdentity(this.authStorage.oauth.accounts(providerId)[0]);
	}
	...
	async #collectBuiltInModelManagerOptions(
		...
		for (let i = 0; i < standardProviderDescriptors.length; i++) {
			...
			const hasStoredOAuth = this.authStorage.credentials.hasOAuth(descriptor.providerId);
