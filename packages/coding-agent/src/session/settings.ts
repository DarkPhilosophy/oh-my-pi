...
export const cfgProvidersOpenaiCodexUseReserve = register({
	id: "providers.openai-codex.useReserve",
	type: "boolean",
	default: false,
	ui: {
		tab: "providers",
...
export const cfgProvidersAnthropicSlowMode = register({
	...
	ui: {
		tab: "model",
		group: "Sampling",
		label: "Slow Mode — Anthropic",
		description:
			"Claude subscription slow mode (same as /slow on Anthropic models). Auto continues at lower priority after the 5-hour session limit when Anthropic offers it; weekly limits still apply.",
		options: [
			{ value: "off", label: "Off", description: "Wait for the Claude usage limit to reset" },
			{ value: "auto", label: "Auto", description: "Switch to lower-priority service at the session limit" },
		],
	},
});
...
export const cfgAcpExposeThinkTool = register({
	id: "acp.exposeThinkTool",
	type: "boolean",
	default: false,
	ui: {
		tab: "providers",
