...
export async function visitEntriesFromFileStream(
	...
	const drain = async (): Promise<void> => {
		...
		while (buffer.length > 0 && !stopped) {
			...
			if (error) {
				...
				// earlier values.
				// advance(), not a bare subarray: the sink only drops bytes that
				// `consumed` accounts for, so skipping it would replay them.
			...
			advance(read);
			// `done` only means the parser reached the end of what it consumed;
			// bytes may remain in this buffer (adjacent records on one physical
			// line). Discarding them would drop entries, so only stop when the
	...
	try {
		...
		// The byte budget is enforced here instead of with `file.slice(0, maxBytes)`:
		// a sliced blob stream stalls on the unaligned tail read of a large file, so
		// the plain stream is read and the chunk that crosses the limit is clipped.
		let bytesRead = 0;
