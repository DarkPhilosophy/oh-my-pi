...
/** Shifts `[Image #N]` markers in text by the given offset. */
export function shiftQueuedImageMarkers(text: string, offset: number): string {
...
/** Extracts the editor-draft text from a queued user message (undefined if not user-authored). */
export function queuedUserDraftText(message: AgentMessage): string | undefined {
...
/** Replaces a queued user message's content with merged text and images. */
export function withQueuedUserContent(message: UserQueueMessage, text: string, images?: ImageContent[]): AgentMessage {
