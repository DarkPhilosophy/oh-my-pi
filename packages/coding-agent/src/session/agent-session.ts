...
import type {
	...
	FreshSessionResult,
	HandoffResult,
	LocalQueueCoalescedListener,
	ModelCycleResult,
	Prewalk,
	PromptOptions,
	QueuedUserMessageListener,
	QueueMode,
...
export class AgentSession implements SettingsScope {
	...
	#watchSessionSettings(): void {
		...
		cfgAdvisorRuntimeInputs.listen(this, (next, previous) => {
			// A budget/cadence/tier edit rebuilds a running advisor (all are part of its
	...
	#processAgentEvent = async (event: AgentEvent, renderSimulation = false): Promise<void> => {
		...
		if (renderSimulation) {
			if (event.type === "message_end") {
				if (messageEndPersistence) await messageEndPersistence;
				else this.#persistMessageEnd(event.message, eventPromptGeneration);
			}
			return;
		}
	...
	async #doDispose(options: AgentSessionDisposeOptions = {}): Promise<void> {
		...
		try {
			await emitSessionShutdownEvent(this.#extensionRunner, options.reason);
		} catch (error) {
