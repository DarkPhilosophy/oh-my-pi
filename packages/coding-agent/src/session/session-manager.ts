		...
		try {
			...
			do {
				...
				try {
					...
					if (actual !== body) {
						// The write this repair recovers from can have torn: a partial
						// line of the very entry being persisted is on disk, so the CAS
						// token predates bytes nobody else wrote. Republishing is still
						// lossless exactly when everything durable is a prefix of the
						// body about to replace it; anything else is a foreign writer.
						if (!(error instanceof SessionWriteConflictError) || !body.startsWith(actual)) {
							recoveryErrors.push(new Error("Authoritative session repair did not match durable storage."));
							throw this.#latchIndeterminate(operationError, recoveryErrors);
						}
						try {
							await this.#storage.writeTextAtomic(sessionFile, body, {
								expectedSize: Buffer.byteLength(actual, "utf8"),
								commitGuard: () => !this.#released && this.#diskEpoch === epoch,
							});
						} catch (retryFailure) {
							recoveryErrors.push(
								toError(retryFailure),
								new Error("Authoritative session repair did not match durable storage."),
							);
							throw this.#latchIndeterminate(operationError, recoveryErrors);
						}
					}
