...
export class TurnRecovery {
	...
	async #maybeApplyUsageAwareFallback(signal: AbortSignal, confirmer?: UsageFallbackConfirmer): Promise<boolean> {
		...
		try {
			...
			if (currentModel.reserveRoute && cfgProvidersOpenaiCodexUseReserve.get(this.#host.settings)) {
				const reserve = await this.#host.modelRegistry.authStorage.oauth.reserveCredential(
