...
import {
	...
	cfgProvidersOpenaiCodexUseReserve,
...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (mcpManager && cfgMcpNotifications.get(settings)) {
			mcpManager.setNotificationsEnabled(true);
		}
		...
		agent = new Agent({
			...
			getApiKey: requestModel =>
				...
				cfgProvidersOpenaiCodexUseReserve.get(settings)
					? undefined
		...
		//     MUST NOT overwrite them. Guarded by `!options.mcpManager && !options.mcpManagerPool`.
		...
		if (mcpManager && !options.mcpManager && !options.mcpManagerPool) {
			mcpCleanup.push(
			...
			mcpCleanup.push(cfgMcpNotifications.listen(session, enabled => mcpManager.setNotificationsEnabled(enabled)));
		}
