...
function writeTerminalTitle(title: string, recomposeStaticOnFailure = false): void {
	...
	// A registered sink (embedded/daemon terminal) owns the title even when this
	// process has no TTY of its own.
	...
	if (terminalTitleSink) {
		terminalTitleSink.setTitle(next);
	} else if (!setWindowsConsoleTitle(next)) {
...
export class TerminalTitleController {
	...
	#startSpinner(): void {
		...
		this.#timer = setInterval(() => {
			this.#frame = (this.#frame + 1) % TERMINAL_TITLE_SPINNER_STYLES.braille.length;
			this.#emit();
...
function startTerminalTitleSpinner(): void {
	if (
		...
		terminalTitleRuntime.nativeTitleFailed ||
		(!process.stdout.isTTY && !terminalTitleSink)
	)
		return;
