...
- `await desktop.windows({app?, title?})` → `[{id, app, title, pid, x, y, width, height, focused}]`; `const win = await desktop.window(selector)` → Win; ambiguous → throws listing candidates. Also `await desktop.focusedWindow()`, `await desktop.displays()`, and `desktop.capabilities()`.
