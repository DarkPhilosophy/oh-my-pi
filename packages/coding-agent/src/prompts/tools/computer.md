...
<rules>
- PREFER AX over pixels: `const win = await desktop.window(selector); await win.ax()` → `el.press()`/`el.click()`/`el.setValue()`. Element actions need no screenshot.
- Pointer `x,y`: pixels in the MOST RECENT screenshot of the SAME target (window or desktop); no target screenshot → coordinate input throws. AX (`.bounds()`, `elementAt`): global desktop coordinates. Spaces differ; both auto-converted; NEVER mix.
...
- Input defaults to `delivery: "background"`. macOS keyboard input to a multi-window app can throw `BackgroundUnavailable` because the OS accepts only a process id and may target another window; use AX or retry `delivery: "foreground"`, which briefly activates the target and restores focus. Targets dropping other background events can also throw `BackgroundUnavailable`, naming the window class and event kind. NEVER infer a background action landed from absent error.
- Wayland: per-window native input and `.raise()` are unavailable; use AX, or desktop input after focusing the target yourself.
- `read_only: true`: pure inspection; input and mutation throw; lighter approval.
