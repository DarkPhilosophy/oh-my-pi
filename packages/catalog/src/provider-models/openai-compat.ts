 ...
 *
 * The default transport is fetched fully once per process: concurrent callers
 * share the in-flight request, repeat callers send a conditional GET that the
 * server answers (and deliberately does not log) with `304`. Caller-supplied
 * transports and abort signals remain isolated so one provider timeout cannot
 * cancel or supply another provider's discovery.
 */
...
function createOpenAICompatibleModelManagerOptions<TApi extends Api>(
	...
	const hasApiKey = hasSimpleProviderApiKey(options.config);
	return {
		providerId: options.providerId,
		...(options.dynamicModelsAuthoritative && { dynamicModelsAuthoritative: true }),
		...((!options.requireApiKey || hasApiKey) && {
...
export function xaiModelManagerOptions(config?: XaiModelManagerConfig): ModelManagerOptions<"openai-responses"> {
	...
	return {
		...createOpenAICompatibleModelManagerOptions({
			...
			config,
			requireApiKey: true,
			filterModel: (_entry, model) => !Object.hasOwn(XAI_CANONICALIZED_ALIAS_IDS, model.id),
			mapModel: mapWithBundledReference,
		}),
...
export function aiandModelManagerOptions(config?: AiandModelManagerConfig): ModelManagerOptions<"openai-completions"> {
	...
	const staticModels =
		baseUrl === AIAND_DEFAULT_BASE_URL
			? AIAND_STATIC_MODELS
			: AIAND_STATIC_MODELS.map(model => ({ ...model, baseUrl }));
	return {
		providerId: "aiand",
		dynamicModelsAuthoritative: true,
