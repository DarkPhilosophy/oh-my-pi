...
describe("github copilot model limits mapping", () => {
	...
	it("reuses a lazy OAuth account cache but isolates a different OAuth account", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "pi-ai-copilot-oauth-cache-"));
		const cacheDbPath = path.join(tempDir, "models.db");
		try {
			const personalFetch = vi.fn(async (input: string | URL | Request) => {
				const url = typeof input === "string" ? input : input.toString();
			...
			const personalManager = createModelManager({
				...githubCopilotModelManagerOptions({
					getApiKey: async () => "ghu_personal_oauth_token",
					cacheIdentity: "account:personal",
					fetch: personalFetch,
				}),
				cacheDbPath,
			});
			...
			const sameAccountFetch = vi.fn(async () => {
				throw new Error("fresh same-account cache must avoid discovery");
			});
			const sameAccountManager = createModelManager({
				...githubCopilotModelManagerOptions({
					getApiKey: async () => "ghu_refreshed_personal_oauth_token",
					cacheIdentity: "account:personal",
					fetch: sameAccountFetch,
				}),
				cacheDbPath,
			});
			const { models: cachedModels } = await sameAccountManager.refresh("online-if-uncached");
			expect(sameAccountFetch).not.toHaveBeenCalled();
			expect(cachedModels.some(model => model.baseUrl === "https://api.githubcopilot.com")).toBe(true);
			...
			const businessManager = createModelManager({
				...githubCopilotModelManagerOptions({
					getApiKey: async () => "ghu_business_oauth_token",
					cacheIdentity: "account:business",
					fetch: businessFetch,
				}),
				cacheDbPath,
			});
			const { models: businessModels } = await businessManager.refresh("online-if-uncached");
			expect(businessSeen).toContain("https://api.github.com/copilot_internal/user");
			expect(businessSeen).toContain("https://api.business.githubcopilot.com/models");
			expect(businessModels.some(model => model.baseUrl === "https://api.business.githubcopilot.com")).toBe(true);
		} finally {
			await fs.rm(tempDir, { recursive: true, force: true });
		}
	});
