...
export class AuthStorage {
	...
	#compose(store: AuthCredentialStore, sourceLabel: string | undefined): AuthStorageModules {
		...
		const limits = new RateLimits({
			...
			usage,
			strategies,
			// `keys` is assigned below; this runs only after construction completes.
			resolveKey: (provider, sessionId, keyOptions) => keys.get(provider, sessionId, keyOptions),
		});
		...
		const oauth = new OAuthAccounts({ pool, overrides, policies, selector, affinity, refresher, usage });
