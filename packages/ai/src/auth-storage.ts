...
export class AuthStorage {
	...
	#reconcileUsageBlockForCredential(provider: Provider, credentialId: number, report: UsageReport): void {
		...
		const scopesToHeal = [undefined, ...blockScopes] as const;
	...
	async rotateSessionCredential(
		...
		const accountPolicyLookup = accountPolicy || AIError.codexChatGPTAccountPolicyModel(error) !== undefined;
		let sessionCredential = await this.#resolveCredentialTarget(provider, sessionId, {
			credentialId: options?.credentialId,
			apiKey: options?.apiKey,
			allowStaleOAuthBearer: accountPolicyLookup,
		});
		if (
			!sessionCredential &&
			sessionId !== undefined &&
