...
export interface OAuthAccountIdentity {
	/** Durable storage row id; scopes the identity when no workspace fields exist. */
	credentialId?: number;
	accountId?: string;
	email?: string;
	projectId?: string;
...
export interface OAuthApi {
	...
	/**
	 * Pick an OAuth account whose exact reserve meter (`route.tier` for
	 * `route.model`) still allows traffic: the session's active account first,
	 * then eligible siblings. Returns the account and the report timestamp the
	 * decision was based on, or `undefined` when no account has reserve quota.
	 */
	reserveCredential(
		provider: string,
	...
	/**
	 * Quarantine a reserve observation the provider rejected, without blocking
	 * the account's normal usage pool. The account becomes eligible for reserve
	 * again once a newer usage report arrives.
	 */
