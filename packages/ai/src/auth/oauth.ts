...
export interface OAuthAccountsDeps {
	...
	/** Usage reports that gate reserve-meter routing ({@link OAuthAccounts.reserveCredential}). */
	usage?: UsageService;
}
...
export class OAuthAccounts implements OAuthApi {
	...
	/** `provider:model:credentialId` → report timestamp at which the provider rejected that reserve observation. */
	...
	identity(provider: string, sessionId?: string): OAuthAccountIdentity | undefined {
		...
		const credentialEntry = this.#deps.pool.entries(provider).find(entry => entry.credential === preferred);
		if (credentialEntry) identity.credentialId = credentialEntry.id;
	...
	async reserveCredential(
		...
		const accounts = this.accounts(provider, options?.sessionId);
		...
		const reports = await usage.reports({ signal: options?.signal });
		const now = Date.now();
