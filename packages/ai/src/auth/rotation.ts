...
export interface RateLimitsDeps {
	...
	/**
	 * Resolve (and thereby record) the credential a request for this session
	 * would use. Lets a denial that arrives before any sticky selection exists
	 * block the account that actually refused, instead of bailing out.
	 */
	resolveKey?: (
		provider: string,
		sessionId: string,
...
/**
 * Model-entitlement denials ("this model is not supported with this account")
 * describe account configuration, not a rate window, so they hold far longer
 * than the generic backoff. A 60s block let the unentitled account become the
 * earliest-unblocking candidate again and the session bounced back onto it
 * before the entitled account was ever used.
 */
const MODEL_POLICY_BLOCK_MS = 12 * 60 * 60 * 1000;
...
export class RateLimits implements LimitsApi {
	...
	async rotate(provider: string, sessionId: string | undefined, options?: RotateCredentialOptions): Promise<boolean> {
		...
		let sessionCredential = await this.#resolveCredentialTarget(provider, sessionId, {
			credentialId: options?.credentialId,
			apiKey: options?.apiKey,
			allowStaleOAuthBearer: accountPolicy || deniedModel !== undefined,
		});
		if (
			!sessionCredential &&
			(accountPolicy || deniedModel !== undefined) &&
			sessionId !== undefined &&
			options?.credentialId === undefined &&
			options?.apiKey === undefined &&
			this.#deps.resolveKey
		) {
			...
			await this.#deps.resolveKey(provider, sessionId, { modelId: options?.modelId, signal: options?.signal });
		...
		if (exactModelPolicy || accountPolicy) {
			...
			return this.#blockCredentialForRotation(
				...
				Date.now() + (exactCodexModelPolicy ? MODEL_POLICY_BLOCK_MS : DEFAULT_BLOCK_MS),
				routing,
				false,
			).switched;
		}
