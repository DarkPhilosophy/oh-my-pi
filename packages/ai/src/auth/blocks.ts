...
export class CredentialBlocks implements BlocksApi {
	...
	/** Active block for one exact scope, excluding legacy catch-all blocks and sibling scopes. */
	exactScopeBlockedUntil(
		provider: string,
		providerKey: string,
		credentialIndex: number,
		blockScope: string,
		...
		const persisted = this.#readPersistedCredentialBlock(credentialId, providerKey, blockScope);
		if (persisted !== undefined && (blockedUntil === undefined || persisted > blockedUntil)) blockedUntil = persisted;
		return blockedUntil;
	}
