...
describe("AuthStorage codex oauth ranking", () => {
	...
	test("routes reserve independently of exhausted chat and Spark pools and binds its account", async () => {
		...
		const selected = await authStorage.oauth.reserveCredential("openai-codex", route);
		expect(selected?.credentialId).toBe(
			authStorage.oauth.accounts("openai-codex").find(account => account.accountId === "reserve")?.credentialId,
		);
		if (!selected) throw new Error("reserve account was not selected");
		const access = await authStorage.oauth.accessById("openai-codex", selected.credentialId);
		expect(access).toMatchObject({ ok: true, accountId: "reserve" });
		authStorage.oauth.rejectReserveCredential("openai-codex", route, selected);
		expect(await authStorage.oauth.reserveCredential("openai-codex", route)).toBeUndefined();
	...
	test("a pinned Spark window neither exhausts nor blocks the account for shared-window models", async () => {
		...
		await authStorage.limits.markReached("openai-codex", "session-spark-advisor", {
			modelId: "gpt-5.3-codex-spark",
		...
		const markResult = await authStorage.limits.markReached("openai-codex", "session-main-agent", {
			modelId: "gpt-5.6-sol",
