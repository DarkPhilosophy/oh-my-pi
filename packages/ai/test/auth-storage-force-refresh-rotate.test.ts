...
describe("AuthStorage forceRefresh + rotateSessionCredential", () => {
	...
	test("Codex denial naming the upstream rollout SKU still rotates for the requested model id", async () => {
		...
		const first = await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId });
		...
		expect(await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId })).toBe("spark-sibling");
		// A sibling family stays entitled on the denied account.
		expect(await codexStorage.keys.get(CODEX_PROVIDER, "spark-other-model", { modelId: "gpt-5.3-codex" })).toBe(
			"spark-denied",
		);
	});
	...
	test("Codex denial rotates even when the session recorded no sticky credential", async () => {
		...
		expect(await codexStorage.limits.rotate(CODEX_PROVIDER, sessionId, { error: denial, modelId })).toBe(true);
		expect(await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId })).toBe("entitled");
	});
	...
	test("Codex denial rotates to the entitled account even when that account hit its chat cap", async () => {
		...
		expect(await codexStorage.keys.get(CODEX_PROVIDER, quotaSession, { modelId: "gpt-5.2" })).toBe("no-entitlement");
		expect(
			await codexStorage.limits.markReached(CODEX_PROVIDER, quotaSession, {
				modelId: "gpt-5.2",
				apiKey: "entitled",
		...
		expect(await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId })).toBe("no-entitlement");
		expect(await codexStorage.limits.rotate(CODEX_PROVIDER, sessionId, { error: denial, modelId })).toBe(true);
		expect(await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId })).toBe("entitled");
	});
	...
	test("Codex denial naming a different tier does not block the requested base model", async () => {
		...
		expect(await codexStorage.keys.get(CODEX_PROVIDER, sessionId, { modelId })).toBe("tier-a");
