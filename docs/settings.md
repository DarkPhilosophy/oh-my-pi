...
| `task.agentAdvisor`         | record  | `{}`    | Per-agent subagent advisor: agent name → `"on"` / `"off"` / advisor model pattern. Overrides agent frontmatter `advisor`; configured from the `/agents` hub. An ancestor's explicit `/advisor off` takes precedence at runtime.                                                                                                                  |
...
| `display.popupFill`               | boolean | `false`          | Opt in to the message-colored background fill for command, argument and inline model popups.                                                                                                                                                                            |
...
| `providers.openai-codex.useReserve` | boolean | `false`   | Prefer `gpt-reserve` for Luna across accounts with an available independent reserve allowance, then return to normal Luna when none remain. Never replay a failure after output. An available Reserve model is also discoverable in `/model` without enabling this setting; direct Reserve selection does not implicitly fall back to Luna.                                                                                                                                                                                                      |
